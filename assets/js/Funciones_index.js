document.addEventListener("DOMContentLoaded", () => 
    ...
    fetch(`https://dummyjson.com/products?limit=${limit}&skip=${skip}`)
        ...
        productos.forEach((product) => 
        {
            ...
            // que manera de perder tiempo, tratando de que me quede acorde a los colores
            cardDiv.innerHTML = `
            <div class="card mt-3">
                <img src="${product.thumbnail}" class="card-img-top" alt="${product.title}" style="height: 200px; object-fit: cover; background-color:rgba(29, 90, 141,0.3);">
                <div class="card-body d-flex flex-column" style="background-color: rgba(12, 61, 67, 0.8);">
                <h5 class="card-title">${product.title}</h5>
                <p class="card-text">${product.description}</p>
                <p class="card-text fw-bold">Precio: $${product.price}</p>
                <button class="btn btn-success mt-auto" style="background-color: rgba(29, 90, 141, 0.5); border-radius: 30px; width: 60%; margin: auto;">Agregar</button>
                </div>
            </div>
            `;
